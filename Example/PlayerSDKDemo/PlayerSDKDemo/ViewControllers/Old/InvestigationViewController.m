...
- (void)viewDidLoad {
    ...
    [self.manager enableVideo:NO];
    [self.manager enableAudio:NO];
    ...
    self.manager.investigationView = _investigationView;
...
{
    return [self.manager submitInvestigation:xmlString];
}
...
- (void)playerManager:(GSPPlayerManager *)playerManager onDoubleTeacherStatusChange:(GSDTStatus)satus
{
    ...
    _progressHUD = [[MBProgressHUD alloc] initWithView:_investigationView];
    _progressHUD.labelText = NSLocalizedString(subMsg, @"");
    [self.view addSubview:_progressHUD];
    [_progressHUD show:YES];
}
